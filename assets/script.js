document.addEventListener("DOMContentLoaded", function () {
  ...
  fetch("assets/data.json")
    ...
    .then((json) => {
      ...
      items.forEach((item, index) => {
        ...
        if (index === 0) {
          ...
          header.querySelector(".accordion-icon-container").className =
            "accordion-icon-container-active";
        }

        // Close all panels, rotate chevrons and change icon backgrounds
        header.addEventListener("click", () => {
          ...
          allHeaders.forEach((_header) => {
            ...
            const iconContainer = _header.querySelector(
              ".accordion-icon-container-active",
            );
            if (pan) pan.hidden = true;
            if (chevron) chevron.className = "chevron-icon";
            if (iconContainer)
              iconContainer.className = "accordion-icon-container";
          });
          ...
          if (!wasExpanded) {
            ...
            panel.hidden = false;
            chevron.className = "chevron-icon-down";
            const iconContainer = header.querySelector(
              ".accordion-icon-container",
            );
            if (iconContainer)
              iconContainer.className = "accordion-icon-container-active";
          } else if (wasExpanded) {
            chevron.className = "chevron-icon";
          }
        });
    ...
    .catch((err) => {
      container.innerHTML =
        '<p class="error">Greška pri učitavanju podataka.</p>';
      console.error(err);
    });
});
